...
Session 3 beginnt mit `m3gim-mobility-starter.csv`, einer Lehrableitung mit zwölf Aussagen aus sechs Dokumenten und drei Orten. `scripts/build-mobility-starter.py` erzeugt die CSV und die Daten des Referenzbeispiels unter `tools/mobility-starter/`. Der Herkunftsnachweis beschreibt die ergänzten Datumsfelder; Session 2 verwendet ebenfalls die Starter-CSV. Die frühere 40-Zeilen-Datei und ihr Begleittext wurden aus den Downloads entfernt; die Git-Historie bewahrt die Ableitung. Die Folien enthalten die zwei Prompts für Datenanalyse und eine einfache statische Website sowie die lokale Startanleitung; eine separate Übungsanleitung wird nicht angeboten. Ein leerer Ordner mit CSV genügt; zusätzliche Wissensdokumente sind für diesen Einstieg nicht erforderlich. Das Abschlussprojekt bietet eine Edition aus PDFs über TEI XML oder ein Forschungsdashboard aus dem vollständigen veröffentlichten M³GIM-Graphen. Beide Wege nutzen `knowledge/data.md`, `knowledge/research.md` und `knowledge/specification.md`. Die erste Fassung hat eine Hauptansicht und eine Kerninteraktion. Sie läuft mit reinem HTML, CSS und JavaScript über einen lokalen statischen Server, ohne zusätzliche Bibliotheken, Frameworks, Paketinstallation, Build, Backend, Datenbank oder externe Dienste. Die PDF-Konvertierungspakete stehen unter den optionalen Materialien.

Die Titel und IDs wurden am 15. September 2026 im Google-Drive-Kursordner abgeglichen. Session 1 und 2 haben eigene Decks und Skripten. Session 3 und 4 verwenden das bestehende gemeinsame Deck und das zusammengeführte Skript unter der bisherigen Session-3-Dokument-ID. Jede Session erscheint einmal mit Titelbild, knapper Überschrift, direkten Materiallinks und flachen Übungstexten. Abstände ersetzen Trennlinien. Die violette Materialfläche gliedert Downloads nach den tatsächlich vorhandenen Hands-ons. Optionale Vorbereitung, Referenzlösungen und eigenständige Beispiele lassen sich aufklappen. Untertitel und Sprungbuttons im Seitenkopf entfallen. Der Footer enthält Autor, Lizenzen und Quellcode. Uhrzeiten und separate Datumszeilen entfallen. Session 3 führt eine aufbauende Übung an; Session 4 geht in selbständiges Arbeiten über. Das gemeinsame Hauptziel lautet „Use LLMs for coding and build small research tools through Promptotyping“. Knowledge Engineering, Context Engineering und Agentic Engineering bilden den begrifflichen Rahmen. Die Einstiegsübung erzeugt eine Website aus der Starter-CSV. Das Abschlussprojekt verwendet PDFs und vorhandene Transkriptionen für eine TEI-Edition oder den vollständigen veröffentlichten M³GIM-Graphen für ein Dashboard. Der offizielle Veranstaltungsname steht im responsiven Header; Slides und Lecture Notes bilden jeweils eine kompakte Gruppe mit zugeordnetem PDF-Link.
...
`downloads/python-vscode.zip` und `downloads/ai-harness.zip` enthalten jeweils Anleitung, unverändertes Python-Skript und die sieben PDFs im Unterordner `input/`. Die Anleitungen erläutern den Übergang von eigener Skriptausführung über die Ausführung durch einen Agenten zum Programmieren mit einem LLM. Die lokal gepflegten Anleitungen und ihre ZIP-Kopien sind bytegleich; die ursprünglichen Drive-Dateien dokumentieren den früheren Stand. Der Konverter wurde mit allen sieben PDFs ausgeführt und erzeugte 40 PNG-Seiten ohne Fehler.
...
Das PDF-Übungspaket stammt aus dem im Vault gepflegten Lehrpaket „PDF pages as images“. Sein README dokumentiert die Bildquelle, CC-BY-Angabe und Ableitung des PDFs. Das IIIF-Paket enthält synthetische Beispielbilder und `images/schulnachricht.jpg` als Quelle der Übungen zu XML, IIIF, TEI und RDF.

## M³GIM-Übung: Faksimiles, Metadaten und TEI

Die Übung „From Facsimiles to TEI“ steht mit allen Materialzugängen direkt bei Session 2. Teilnehmende wählen ein oder zwei Dokumente oder alle sieben und können bereits einen AI Harness oder alternativ einen LLM-Chat verwenden. Sie transkribieren originalsprachig, prüfen Volltext und Metadaten und erzeugen TEI. Übersetzung ist optional. Das Abschlussprojekt kann vorhandene Transkriptionen, TEI-Dateien und Bilder für eine digitale Edition wiederverwenden; das vollständige Referenzpaket mit sieben Dokumenten und 40 Scanseiten dient als optionaler Rückgriff. Der manuelle PDF-Workflow und sein Agentvergleich sind optionale technische Vorbereitung. Eine kurze Exploration des M³GIM-Prototyps führt von konkreten Belegen und fehlenden Daten oder Funktionen zu einer eigenen Anforderung für Session 4.
...
Titel, Untertitel, Kurzbeschreibungen, Lernziele und Materialverzeichnis liegen in `scripts/sessions.mjs`; `scripts/build-site.mjs` erzeugt daraus die Seiten. Ausführliche Übungsanleitungen stehen im Downloadordner. Nach Änderungen zuerst `uv run scripts/build-m3gim-tei.py`, dann `node scripts/build-site.mjs` ausführen. Der erste Schritt erzeugt TEI und CSV und aktualisiert Anleitungs-, Referenz- und Folgepaket. Rohtranskriptionen liest er aus dem bestehenden Referenzarchiv; die PNGs aus dem bestehenden Bildarchiv.
...
## Vollständiger Datensatz für das Dashboard

`downloads/m3gim-dataset.jsonld` enthält den vollständigen veröffentlichten Archivgraphen aus `DigitalHumanitiesCraft/m3gim`, Revision `bec2a068802e9f2cb9885df072699b269d6efaa9`. Die 4.321 Top-Level-Knoten sind byteidentisch zur Quelle. `m3gim-dataset-source.txt` dokumentiert Struktur, Lizenz und Grenzen; `scripts/build-m3gim-dataset.py` reproduziert und prüft den Snapshot. Die 12-Zeilen-Starter-CSV bleibt ausschließlich der kleine Übungsdatensatz. Rohspreadsheets und Faksimiledateien sind nicht Bestandteil des Graphdownloads.
