...
Static course website for Christopher Pollin's four sessions at Summer School Musicology 2026 (Graz, 16 and 17 September 2026), published by GitHub Pages straight from the root of `main` without any build step on GitHub. Generated HTML, title-slide images and download ZIPs are therefore committed. Slides and lecture notes stay as Google Slides and Docs on Drive and are only linked. Project knowledge follows the Promptotyping convention under `knowledge/` (entry point `knowledge/INDEX.md`, work diary `knowledge/journal.md`); consult `specification.md` there before conceptual changes, it records the design decisions. Course page texts and teaching instructions are English, README and journal are German.
...
node scripts/build-site.mjs --check    # render without writing, fail on drift or broken local links/anchors
...
python scripts/build-shared-packages.py  # rebuild the per-session packages, xml-iiif-workshop, szd-facsimiles, ai-harness, python-vscode and shared-materials ZIPs, refresh hashes in downloads/drive-materials.json
