...
The course page opens with the event, dates and venue, followed by one row per session. Each row pairs its title-slide image on the left with its material links and exercise text on the right. Below 800 pixels, the image appears above the text. There is no separate overview repeating the sessions. Every session has one heading, a plain row of slide and notes links including PDF exports, then one descriptive sentence, learning objectives and material links. Session times, separate date labels, nested cards and slide embeds are omitted. Each session has a compact violet download area on the right: two or three primary packages stay visible, while further sources and references are collapsed under “More materials”. The header carries the full official event name, and the introduction contains the title and byline; its subtitle and jump buttons are omitted.

M³GIM exercise instructions, prompts and references are direct downloads on the course page. The legacy `materials/m3gim-fulltext.html` redirects to Session 2, `#reference` to its optional references and `#next-session` to Session 3. The viewer page holds the file inputs, the Mirador container and three collapsed guides, and its element IDs are the contract with `tools/iiif-viewer/app.js`.
...
Session 3 begins with a minimal AI-harness exercise using `m3gim-mobility-starter.csv`: twelve statements from six documents and three places. The derived fields `date_type` and `date_status` make date meaning and uncertainty explicit; `not_flagged` does not mean verified certainty. `scripts/build-mobility-starter.py` reproducibly builds the CSV and JSON for the fixed-scale reference timeline in `tools/mobility-starter/`. The reference uses native buttons, vertical collision separation and a details panel. Session 2 also uses the starter CSV. The former 40-row CSV and its companion text are removed from downloads; the builder reads its input from Git commit f2d0608. Participants put the CSV in an empty folder, inspect it with Python in a first prompt and build one static HTML/CSS/JavaScript timeline in a second prompt. The slides provide both prompts, local server commands and source checks. There is no separate downloadable guide for this exercise. No `data.md` or `research.md` is required for this introduction. The existing TEI source-viewer exercise follows as a deeper application; PDF conversion packages remain under optional materials. The CSV has one `date` field; `recorded_role` and notes establish its interpretation. Case-insensitive place grouping preserves original spellings.
...
Session 2 allows one or two documents or all seven, processed with an AI Harness or LLM chat in their original languages. Translation is optional. Session 3 reuses the resulting TEI files and images for guided construction of a small static web publication. Manual PDF conversion and the agent comparison remain introductory exercises according to prior experience. Participants briefly explore the M³GIM work-in-progress prototype, record source evidence and missing data or functions, and derive a requirement without treating announcements as proof of appearances. Session 4 extends the publication or another research requirement. The complete reference corpus is an optional fallback. Download labels identify the exercise and the role of each file.
